...
app = Flask(__name__)
app.secret_key = 'your_secret_key'  # Aggiungi una chiave segreta per usare il sistema di messaggi di Flask
...
def aggiungi():
    elemento = request.form.get('elemento')  # Ottieni l'elemento dal form
    if elemento:  # Verifica che l'elemento non sia vuoto
        ...
        flash('Elemento aggiunto con successo!', 'success')  # Mostra un messaggio di successo
    else:
        flash('L\'elemento non può essere vuoto.', 'danger')  # Messaggio di errore
...
def rimuovi(id):
    ...
    db.session.commit()  # Salva i cambiamenti nel database
    flash('Elemento rimosso con successo!', 'success')  # Mostra un messaggio di successo
    return redirect(url_for('home'))
...
def svuota():
    ...
    db.session.commit()  # Salva i cambiamenti nel database
    flash('Lista svuotata con successo!', 'success')  # Messaggio di successo
    return redirect(url_for('home'))
